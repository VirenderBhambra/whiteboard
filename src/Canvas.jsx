...
export default function Canvas() {
  ...
  useEffect(() => {
    ...
    // Wait for the server to signal that it's ready to receive canvas data
    socket.on('server-ready', () => {
      console.log('Server is ready to receive canvas data');
    });
...
// }, [ctx]);
  ...
  const draw = ({ nativeEvent }) => {
    ...
    timeout.current = setTimeout(function () {
      ...
      socket.emit("canvas-data", base64ImageData, (ack) => {
        console.log('Acknowledgment received from server:', ack);
      });
